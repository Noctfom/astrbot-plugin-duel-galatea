...
class YugiohCardSearcher:
    ...
    RACE_MAP = {
        1: "战士", 2: "魔法师", 4: "天使", 8: "恶魔", 16: "不死", 32: "机械", 
        64: "水", 128: "炎", 256: "岩石", 512: "鸟兽", 1024: "植物", 2048: "昆虫", 
        4096: "雷", 8192: "龙", 16384: "兽", 32768: "兽战士", 65536: "恐龙", 
        131072: "鱼", 262144: "海龙", 524288: "爬虫类", 1048576: "念动力", 2097152: "幻神兽"
    }
    ...
    def _add_basic_info(self, card_data: Dict, info: List[str]):
        ...
        name_display = "{} ({})".format(cn_name, sc_name) if sc_name and sc_name != cn_name else cn_name
    ...
    def _add_monster_info(self, data: Dict, types_str: str, text_data: Dict, info: List[str]):
        """辅助方法：添加怪兽详细信息"""
    ...
    def format_search_results(self, results: List[Dict], page: int, user_id: str) -> str:
        page_size = 10
        ...
        output = ["🔍 搜索结果 (第 {}/{} 页，共 {} 个结果):\n".format(page, total_pages, total_results)]
...
class DuelGalateaPlugin(Star):
    def __init__(self, context=None, config: AstrBotConfig = None):
        ...
        self.deck_breakdown = DeckBreakdownManager(str(self.data_dir), self.plugin_source_dir, self.ydk_manager)
        # 加载ID (从源码目录读取)
        self._load_card_ids()
    ...
    def _load_card_ids(self):
        ...
        try:
            ...
            if os.path.exists(ids_file_path):
                ...
                # === 修复日志违规 ===
                logger.info(f"DuelGalatea: 成功加载 {len(self.all_card_ids)} 个卡片ID到随机池")
            else:
    ...
    def _load_backup_ids(self):
        """备用ID列表"""
        backup_ids = [
            "16178681", "89631139", "4064256", "74677422", "38033121",
            "10000000", "53129443", "83104731", "94192409", "53334471",
            "46986414", "70828912", "36935103", "7902349", "65741786"
        ]
    ...
    async def handle_cha_ka(self, event: AstrMessageEvent):
        ...
        if "error" in result:
            ...
            self.search_sessions[user_id] = {"results": result["result"], "current_page": 1, "page_size": 10, "query": query}
            response_text = self.card_searcher.format_search_results(result["result"], 1, user_id)
    ...
    async def handle_select_card(self, event: AstrMessageEvent):
        ...
        if 1 <= card_number <= len(results):
            ...
            if "error" in detail_result:
                await event.send(event.plain_result(f"获取详情失败: {detail_result['error']}"))
                return
    ...
    async def handle_random_card(self, event: AstrMessageEvent):
        ...
        for _ in range(3):
            try:
                ...
                detail_result = await self.card_searcher.get_card_detail(str(random_card_id))
    ...
    async def handle_md_deck_breakdown(self, event: AstrMessageEvent):
        ...
        if deck_name != raw_name:
            await event.send(event.plain_result(f"🔍 识别到中文卡组名【{raw_name}】，自动转换为【{deck_name}】进行查询..."))
        else:
            await event.send(event.plain_result(f"🔍 [MDM] 正在抓取【{deck_name}】数据并生成构筑图，请稍候..."))
        # === 修改结束 ===
    ...
    async def handle_dl_deck_breakdown(self, event: AstrMessageEvent):
        ...
        if deck_name != raw_name:
            ...
            await event.send(event.plain_result(f"🔍 [DLM] 正在抓取【{deck_name}】数据并生成构筑图，请稍候..."))
        # === 修改结束 ===
    ...
    async def handle_send_ydk(self, event: AstrMessageEvent):
        ...
        if os.path.exists(path):
            ...
            await event.send(event.chain_result([Comp.File(name=os.path.basename(path), file=path)]))
        else:
    ...
    async def handle_receive_ydk(self, event: AstrMessageEvent):
        ...
        await event.send(event.plain_result(f" YDK 已接收 (M:{len(main)} E:{len(extra)} S:{len(side)})。你可以使用 /发送卡组图片 查看。"))
    
    @filter.command("接收卡组链接", alias=["/接收卡组链接","解析卡组链接","/解析卡组链接"])
    async def handle_receive_deck_link(self, event: AstrMessageEvent):
        ...
        if url.startswith("ydke://"):
            ...
            await event.send(event.plain_result("⚠️ 未知链接格式。支持：\n1. deck.ourygo.top 分享链接\n2. ydke:// 代码"))
            return
        ...
        # 3. 生成图片
        await event.send(event.plain_result(f"✅ [{source_type}] 解析成功 (M:{len(main)} E:{len(extra)} S:{len(side)})\n🎨 正在绘图..."))
        img_path = await self.ydk_manager.draw_deck_image(session_id,f"Shared {source_type}")
        
        if img_path:
            await event.send(event.image_result(img_path))
        else:
            await event.send(event.plain_result("⚠️ 图片生成失败，但 YDK 已保存。可以使用 /发送ydk 获取。"))