...
class GenericTierManager:
    ...
    def get_data_file_path(self, game_type: GameType) -> str:
        return os.path.join(self.plugin_path, f"{game_type.value}_tier_data.json")

    def get_translations_file_path(self) -> str:
        return os.path.join(self.plugin_path, "deck_translations.json")
    ...
    async def batch_translate_and_save(
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            for deck in targets:
                ...
                tasks.append(await self.get_chinese_name(session, deck, force_api=True))
    ...
    def _parse_md_data(self, content: str) -> Dict[str, List[str]]:
        ...
        print(f"   -> [解析结果] T1:{t1_len}, T2:{t2_len}, T3:{t3_len}")

        # 如果还是空，尝试 fallback (可能没有图片 alt，只有文本)
        if t1_len + t2_len + t3_len == 0:
            print("⚠️ [MD Parse] 标准模式为空，尝试文本匹配...")
            # 备用方案：直接找文本 >Tier 1<
    ...
    async def _async_crawl_tier_data(
        ...
        print(f"🔍 开始异步爬取 {game_type.value} T表...")
        urls = {
    ...
    async def crawl_tier_data(self, game_type: GameType) -> Optional[TierData]:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            try:
                ...
                print(f"[Tier] 爬取或翻译流程失败: {e}")
                return None


class TierCommandHandler:
    def __init__(self, plugin_path: str):
        self.manager = GenericTierManager(plugin_path)
