...
class RotKManager:
    def __init__(self, plugin_path: str):
        ...
        self.data_file = os.path.join(plugin_path, "ocg_report_data.json")
        self.img_dir = os.path.join(plugin_path, "ocg_images")
    ...
    async def _fetch_html(self, session, url):
        try:
            ...
            print(f"[RotK] Fetch error {url}: {e}")
        return None
    ...
    async def _download_images(self, url_list):
        print("[RotK] Clearing cache and downloading images...")
    ...
    async def _fetch_article_content_images(self, session, article_url):
        """二级跳：精准提取正文图片"""
        print(f"[RotK] Deep fetching: {article_url}")
        ...
        for img in raw_imgs:
            ...
            if re.search(r"-\d{3}x\d{2,3}\.", clean_url):
                print(f"[RotK] Skipping thumbnail: {clean_url}")
                continue
        ...
        print(f"[RotK] Found {len(valid_imgs)} content images")
        return valid_imgs

    async def fetch_latest_report(self):
        print(f"[RotK] Starting fetch from {self.base_url}")
        async with aiohttp.ClientSession(headers=self.headers) as session:
