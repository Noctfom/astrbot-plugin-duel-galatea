...
class YDKManager:
    ...
    def _decode_ourocg_data(self, encoded_str: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.info(
                f"Ourocg Decode: M:{len(main_ids)} E:{len(extra_ids)} S:{len(side_ids)}"
            )
    ...
    def parse_ydke_url(self, url: str) -> Tuple[List[str], List[str], List[str]]:
        ...
        try:
            ...
            logger.info(
                f"YDKe Decode: M:{len(main_ids)} E:{len(extra_ids)} S:{len(side_ids)}"
            )
    ...
    def save_ydk(
        self, main: List[str], extra: List[str], side: List[str], session_id: str
    ) -> str:
        """保存 YDK (区分会话)"""
    ...
    async def draw_deck_image(
        self, session_id: str, deck_name: str = "YDK Deck"
    ) -> Optional[str]:
        if not HAS_PILLOW:
            return None
        ...
        logger.info(
            f"🎨 Drawing YDK: Main({len(main)}) Extra({len(extra)}) Side({len(side)})"
        )
        ...
        try:
            ...
            if extra:
                total_h += section_gap + (extra_rows * (card_h + gap))
            if side:
                total_h += section_gap + (side_rows * (card_h + gap))
            total_h += 20  # Padding
            ...
            draw.text(
                (total_w - 200, 12),
                f"M:{len(main)} E:{len(extra)} S:{len(side)}",
                font=font,
                fill=(200, 200, 200),
            )
            ...
            if extra:
                ...
                draw.line(
                    [
                        (gap, current_y - section_gap / 2),
                        (total_w - gap, current_y - section_gap / 2),
                    ],
                    fill=(60, 60, 60),
                    width=2,
                )
